...
const EventSetup = {
    ...
    saveDocument: {id: 'save_document', fn: newDocumentOption, action: 'click', args: ['docSelect', 'new_document_name']},
    docDisplays: { className: 'doc-display' },
    newDocName: { id: 'new_document_name', },
    deleteDocument: { id: 'delete_document', },
}
...
function updateAmount(e, globalConstString = '', type = '', callback = () => { }, colNames = [], eventCallback = () => { },) {
    ...
    for (x = InputTable.children.length; x <= ref.value; x++) {
        //rowNode.setAttribute('data-index', x-1);
        InputTable.insertBefore(rowNode.cloneNode(true), InputTable.lastElementChild);
...
function newDocumentOption(e, globalConstString = '', nameInput,){
    ...
    if(nameInput){
        if(window[nameInput]){
            if(window[nameInput].value.length > 1){
...
function addRow(e, globalConstString = '', ){
    ...
    //populate the DOM with 2 rows on load
        incomeAmount.value = 2
        accountAmount.value = 2
        expenseAmount.value = 2
        contactAmount.value = 2
...
function selectDocument(e){
    ...
    if(targetDoc === ""){
        newDocName.value = '';
        newDocName.style.opacity = 100;
        deleteDocument.style.opacity = 100;
        saveDocument.style.opacity = 100;
        importantDocs.labels[0].style.opacity = 0;
    }else{
        newDocName.style.opacity = 0;
        deleteDocument.style.opacity = 0;
        saveDocument.style.opacity = 0;
        importantDocs.labels[0].style.opacity = 100;
    }
};
...
function formDisplayUpdate(json) {
                    ...
                    if (typeof data[key] === 'object') {
                        for(const [index, rows] of data[key].entries()){
                            ...
                            for (const resource of Object.keys(rows)) {
                                const parentCols = document.querySelectorAll(`div.col-3[data-storage='${resource}']`)
                                //const Input = document.querySelector(`div.row[data-index='${index}'] ${fillPairs[resource]}`);
                                const Input = parentCols[index].children[0];
    ...
    if (json.documents?.length) {
        ...
        if(docDataObject[targetDoc]) updateDocDisplay(docDataObject[targetDoc]);
